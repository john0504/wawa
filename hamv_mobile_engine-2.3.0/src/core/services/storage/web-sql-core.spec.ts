...
import { WebSQLMock } from '../../../mocks/websql.mocks';
...
describe('WebSql core - basic testing', () => {

  let instance: WebSqlCore;

  beforeAll(() => {
    spyOn(win, 'openDatabase').and.callFake(() => {
      return new WebSQLMock();
    });
    instance = new WebSqlCore();
  });
...
describe('WebSql core - query testing', () => {
  ...
  afterAll(() => {
    return instance.deleteStorage().then(() => instance.stop())
      .catch(e => {
        expect(e).toBeDefined();
        expect(e).toEqual('Not supported');
      });
  });
...
describe('WebSql core - selection testing', () => {
