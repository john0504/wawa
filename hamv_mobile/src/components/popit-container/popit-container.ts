...
export class PopitContainerComponent implements OnInit, OnDestroy {
  ...
  private updateUi([devices, account]) {
    ...
    if (this._deviceSn && devices[this._deviceSn]) {
      ...
      this.buttonLength = this.getButtonsLength();
    }
  }
  ...
  private getButtonsLength() {
    // The default length would equal to buttons' length which are always exist on the UI
    let buttonLength = 1;
    const checkDatum = [
      this.isOwner,
      this.deviceCore.hasHistoryComponents
    ];
    checkDatum.forEach((data) => {
      if (data) {
        buttonLength += 1;
      }
      return buttonLength;
